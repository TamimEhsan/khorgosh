...
inline float warmup_ip_x0_q_512(
    ...
    for (; i < dim_end_512; i += 512) {
        ...
        // Force complete unrolling of the inner dimension loop
        #pragma GCC unroll 8
        for (size_t j = 0; j < b_query; ++j) {
            __m512i query_vec = _mm512_loadu_si512(query + (j * 8)); // Optimized index access
            __m512i pop = _mm512_popcnt_epi64(_mm512_and_si512(data_vec, query_vec));
            acc_bits[j] = _mm512_add_epi64(acc_bits[j], pop);
        }
        query += b_query * 8; // Advance outer query pointer all at once
    }
    ...
    if (remaining_dim > 0) {
        ...
        #pragma GCC unroll 8
        for (size_t j = 0; j < b_query; ++j) {
            __m512i query_vec = _mm512_maskz_loadu_epi64(valid_mask, query + (j * num_chunks));
            __m512i pop = _mm512_popcnt_epi64(_mm512_and_si512(data_vec, query_vec));
            acc_bits[j] = _mm512_add_epi64(acc_bits[j], pop);
        }
    }
